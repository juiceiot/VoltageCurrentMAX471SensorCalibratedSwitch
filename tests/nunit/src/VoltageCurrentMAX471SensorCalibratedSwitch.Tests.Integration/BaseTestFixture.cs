...
namespace VoltageCurrentMAX471SensorCalibratedSwitch.Tests.Integration
{
	public class BaseTestFixture
	{
		...
		{
			var devicePort = Environment.GetEnvironmentVariable("PROTECTOR_PORT");
		...
		{
			var simulatorPort = Environment.GetEnvironmentVariable("PROTECTOR_SIMULATOR_PORT");
		...
		{
			var baudRateString = Environment.GetEnvironmentVariable ("PROTECTOR_BAUD_RATE");
		...
		{
			var baudRateString = Environment.GetEnvironmentVariable ("PROTECTOR_SIMULATOR_BAUD_RATE");
